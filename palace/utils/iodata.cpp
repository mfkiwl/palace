...
void IoData::NondimensionalizeMesh(mfem::ParMesh &mesh) const
{
  // Scale mesh vertices for correct nondimensionalization.
  for (int i = 0; i < mesh.GetNV(); i++)
  {
    double *v = mesh.GetVertex(i);
    std::transform(v, v + mesh.SpaceDimension(), v, [this](double val) { return val / (Lc / model.L0); });
  }
  if (mesh.GetNodes())
  {
...
void IoData::DimensionalizeMesh(mfem::ParMesh &mesh) const
{
  // Scale mesh vertices for correct nondimensionalization.
  for (int i = 0; i < mesh.GetNV(); i++)
  {
